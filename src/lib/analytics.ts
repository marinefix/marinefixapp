...
async function send(event: "visit" | "session" | "install", path: string): Promise<void> {
  try {
    ...
    // Analytics must never break the app when offline or when the API is unavailable.
  }
}

export function trackUsage(path = window.location.pathname): void {
  ...
  if (firstLaunch) {
    ...
    if (isAndroidApp()) void send("install", path);
  }

  // One session event per visitor per 24 hours keeps D1 storage small.
  const lastSession = Number(localStorage.getItem(LAST_SESSION_KEY) || "0");
  if (!Number.isFinite(lastSession) || Date.now() - lastSession >= 24 * 60 * 60 * 1000) {
