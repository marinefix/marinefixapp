...
async function cacheGuideAttachments(
  ...
  await Promise.all(
    getAttachmentUrls(guide).map(cacheAttachment)
  );
}
...
export function getOfflineGuides(): Record<
  string,
  OfflineGuide
> {
  try {
...
export function isGuideSavedOffline(
  guideId: string
): boolean {
  return Boolean(
    getOfflineGuides()[guideId]
  );
}

export async function getOfflineAttachmentUrl(
  url: string
): Promise<string> {
  const absoluteUrl = resolveRemoteUrl(url);
  ...
  try {
    ...
    console.warn(
      "Failed to read cached attachment:",
      absoluteUrl,
      err
    );
...
export async function clearOfflineGuides(): Promise<void> {
  try {
    ...
    console.error(
      "Failed to clear offline guides:",
      err
    );
  }
}